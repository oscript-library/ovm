...
Функция МокОкружения(Знач ВидПлатформы, Знач Архитектура)
	...
	МокСИ.Когда().ЭтоWindows().ТогдаВозвращает(СтрНачинаетсяС(ВидПлатформы, "Windows"));
	МокСИ.Когда().ЭтоLinux().ТогдаВозвращает(СтрНачинаетсяС(ВидПлатформы, "Linux"));
	МокСИ.Когда().ЭтоMacOS().ТогдаВозвращает(СтрНачинаетсяС(ВидПлатформы, "MacOS"));

	МокСИ.Когда().ЭтоMacOSARM().ТогдаВозвращает(
		СтрНачинаетсяС(ВидПлатформы, "MacOS")
		И Архитектура = АрхитектурыПроцессоров.ARM64 Или Архитектура = АрхитектурыПроцессоров.ARM
	);
