...
Процедура ПриЗагрузкеБиблиотеки(Путь, СтандартнаяОбработка, Отказ) // BSLLS:UnusedLocalMethod-off
	Вывести("
	|ПриЗагрузкеБиблиотеки " + Путь);	

	Если Осень <> Неопределено Тогда
		ПутьДоЗаготовки = ОбъединитьПути(Путь, "src/Заготовки/ЗаготовкаCli.os");
		ПодключитьСценарий(ПутьДоЗаготовки, "ЗаготовкаCli");
		Осень.ДобавитьЗаготовкуДляАвтоИнициализации("ЗаготовкаCli");
...
Процедура ДобавитьКлассЕслиРанееНеДобавляли(ПутьФайла, ИмяКласса)
	...
	КлассУжеЕсть = Ложь;
	Попытка
		Объект = Новый(ИмяКласса); // BSLLS:UnusedLocalVariable-off
		КлассУжеЕсть = Истина;
	Исключение
	...
	Если ЗначениеЗаполнено(РезИзСреды) Тогда
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				"Неверный формат переменной среды %1. Ожидали 1 или 0, а получили %2",
				ИмяПеременнойСреды,
				РезИзСреды
			);
...
// что помогает при анализе проблем
