...
&Пластилин
Перем ВерсииOneScript; // Работа с версиями OneScript
...
Процедура ОписаниеКоманды(КомандаПриложения) Экспорт
	...
	КомандаПриложения.Аргумент(
		...
		"Устанавливаемая версия (версии) OneScript. "
			...
			+ "а также сборки веток и пул-реквестов с сервера сборок "
			+ "(branch:develop, branch:feature/my-branch, pr:1731). "
			+ "Конкретную сборку можно указать через решетку: branch:develop#777. "
...
Процедура ВыполнитьКоманду(Знач КомандаПриложения) Экспорт
	...
	Если МассивВерсийКУстановке.Количество() > 0 Тогда
		...
		Если НЕ ЗначениеЗаполнено(АлиасВерсии) Тогда
			АлиасВерсии = ВерсииOneScript.АлиасПоУмолчанию(
