...
&Опция(
	Имя = "builds b",
	Описание = "Вывести список сборок веток и пул-реквестов, доступных к установке с сервера сборок")
&ВОкружении("OVM_LIST_BUILDS")
&ТБулево
&ПоУмолчанию(Ложь)
Перем ВыводитьСписокСборок; // Признак вывода списка сборок с сервера сборок
...
Процедура ВыполнениеКоманды() Экспорт
	...
	Если ВыводитьСписокСборок Тогда
		ВывестиСписокДоступныхКУстановкеСборок();
	ИначеЕсли ВыводитьВсеВерсии Тогда
		ВывестиВсеВерсии();
...
Процедура ВывестиСписокДоступныхКУстановкеСборок()

	СписокДоступныхСборок = ВерсииOneScript.ПолучитьСписокДоступныхКУстановкеСборок();
	Для Каждого ДоступнаяСборка Из СписокДоступныхСборок Цикл
		Если ТихийРежим Тогда
			Сообщение = ДоступнаяСборка.Алиас;
		Иначе
			Сообщение = СтрШаблон("%1 (%2)", ДоступнаяСборка.Алиас, ДоступнаяСборка.Путь);
		КонецЕсли;
		Лог.Информация(Сообщение);
	КонецЦикла;
