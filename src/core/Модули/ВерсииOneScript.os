...
Функция ВерсияУстановлена(Знач ПроверяемаяВерсия) Экспорт
	...
	Результат = Результат И (
		ФС.ФайлСуществует(ОбъединитьПути(КаталогУстановкиВерсии, "bin", "oscript.exe"))
		Или ФС.ФайлСуществует(ОбъединитьПути(КаталогУстановкиВерсии, "bin", "oscript.bat"))
		Или ФС.ФайлСуществует(ОбъединитьПути(КаталогУстановкиВерсии, "bin", "oscript.sh")));
...
Функция ПолучитьСписокДоступныхКУстановкеВерсий() Экспорт
	...
	Если ТипЗнч(АдресСайтаОСкрипт) = Тип("Массив") Тогда
		...
		Для Каждого мСайт Из АдресСайтаОСкрипт Цикл
			
			Попытка
				ВерсииСайта = ПолучитьСписокДоступныхКУстановкеВерсийССайта(мСайт);
			Исключение
				Лог.Предупреждение(ОписаниеОшибки());
				Продолжить;
			КонецПопытки;

			ДоступныеВерсии = ДополнитьДоступныеВерсии(ДоступныеВерсии, ВерсииСайта);

		КонецЦикла;

	Иначе
		ДоступныеВерсии = ПолучитьСписокДоступныхКУстановкеВерсийССайта(АдресСайтаОСкрипт);
	КонецЕсли;
...
Функция ДополнитьДоступныеВерсии(Знач ДоступныеВерсии, Знач ДоступныеВерсииДобавляемые)

	Если ДоступныеВерсии = Неопределено Тогда
		Возврат ДоступныеВерсииДобавляемые;
	КонецЕсли;

	Для Каждого мВерсия Из ДоступныеВерсииДобавляемые Цикл

		СтрокаВерсии = ДоступныеВерсии.Найти(мВерсия.Алиас, "Алиас");
		Если СтрокаВерсии = Неопределено Тогда
			СтрокаВерсии = ДоступныеВерсии.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаВерсии, мВерсия);
		КонецЕсли;
...
Функция ПолучитьСписокДоступныхКУстановкеВерсийССайта(Знач АдресСайтаОСкрипт)
	...
	ОбеспечитьСтрокуВерсииПоАлиасу(АдресСайтаОСкрипт, ДоступныеВерсии, "dev", "Путь");
	ОбеспечитьСтрокуВерсииПоАлиасу(АдресСайтаОСкрипт, ДоступныеВерсии, "stable", "Путь");
...
Функция ПолучитьСписокВсехВерсий() Экспорт
	...
	ОбеспечитьСтрокуВерсииПоАлиасу(ПараметрыOVM.АдресСайтаОСкриптПоУмолчанию(), ВсеВерсии, "dev");
	ОбеспечитьСтрокуВерсииПоАлиасу(ПараметрыOVM.АдресСайтаОСкриптПоУмолчанию(), ВсеВерсии, "stable");
...
Процедура ОбеспечитьСтрокуВерсииПоАлиасу(АдресСайтаОСкрипт, ТаблицаВерсий, Алиас, ИмяРеквизитаПуть = "ПутьСервер")
	...
	СтрокаВерсии[ИмяРеквизитаПуть] = ПараметрыOVM.ПолныйАдресККаталогуДистрибутивов(АдресСайтаОСкрипт);
