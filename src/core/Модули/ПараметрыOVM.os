...
// Разрядность устанавливаемого дистрибутива OneScript
// (только для x64 систем)
//
//  Возвращаемое значение:
//   Булево - Использовать 32 разрядность
Функция Использовать32бита() Экспорт
	
	ЗначениеПеременнойСреды = ПолучитьПеременнуюСреды("OVM_INSTALL_X86");
	Если НЕ ЗначениеПеременнойСреды = Неопределено Тогда
		Попытка
			Возврат Булево(ЗначениеПеременнойСреды);
		Исключение
			Возврат Ложь;
		КонецПопытки;
	КонецЕсли;

	ЗначениеИзФайлаНастройки = ЗначениеНастройки("oscript.x86", Ложь);

	Возврат ЗначениеИзФайлаНастройки;
