...
Функция АдресСайтаОСкрипт() Экспорт
	Возврат ЗначениеНастройки("oscript.server", АдресаПоУмолчанию());
КонецФункции

// Адрес основного сайта OneScript
//
//  Возвращаемое значение:
//   Строка - Адрес сайта OneScript
//
Функция АдресСайтаОСкриптПоУмолчанию() Экспорт
	
	ЗначениеНастройки = АдресСайтаОСкрипт();

	Если ТипЗнч(ЗначениеНастройки) = Тип("Массив") Тогда
		
		Если ЗначениеЗаполнено(ЗначениеНастройки) Тогда
			Возврат ЗначениеНастройки[0];
		КонецЕсли;
...
Функция АдресаПоУмолчанию()
	АдресаПоУмолчанию = Новый Массив;
	АдресаПоУмолчанию.Добавить("https://oscript.io");
	АдресаПоУмолчанию.Добавить("https://oscript.ru");
	Возврат АдресаПоУмолчанию;
КонецФункции

// Полный адрес к каталогу дистрибутивов OneScript
//
// Параметры:
//  АдресСайтаОСкрипт - Строка - Адрес сайта или зеркала
//
//  Возвращаемое значение:
//   Строка - Полный адрес к каталогу дистрибутивов OneScript
//
Функция ПолныйАдресККаталогуДистрибутивов(Знач АдресСайтаОСкрипт) Экспорт
