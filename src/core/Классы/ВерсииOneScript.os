...
Функция ПолучитьСписокДоступныхКУстановкеВерсий() Экспорт
	...
	ТаблицаВерсийНаБазеAPI = ПолучитьВерсииПоAPI(Соединение);
	Если ТаблицаВерсийНаБазеAPI <> Неопределено Тогда
		Возврат ТаблицаВерсийНаБазеAPI;
	КонецЕсли;
	
	Возврат ПолучитьВерсииРазборомHTML(Соединение);
КонецФункции
...
Функция ПолучитьВерсииРазборомHTML(Знач Соединение)
