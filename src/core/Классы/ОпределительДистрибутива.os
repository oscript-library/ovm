...
Процедура ПриСозданииОбъекта(&Пластилин("ДетекторОкружения") Знач Детектор)
	ДетекторОкружения = Детектор;
	ИспользоватьFDD = Ложь;
	ИспользоватьХ64 = Ложь;
	ЭтоДистрибутивНеткор = Неопределено;
КонецПроцедуры
...
Функция НайтиПодходящийДистрибутив(Знач ПереченьФайлов, Знач ВидДистрибутива, Знач ИспользоватьХ64)
	...
	Лог.Отладка("Ищу дистрибутив по критериям Вид=%1")
