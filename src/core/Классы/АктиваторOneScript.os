...
Процедура ИспользоватьВерсиюOneScript(Знач ИспользуемаяВерсия, Знач ВыполнятьУстановкуПриНеобходимости = Ложь) Экспорт
	...
	ВерсияКУстановке = ИспользуемаяВерсия;
	ИспользуемаяВерсия = ВерсииOneScript.АлиасПоУмолчанию(ИспользуемаяВерсия);

	Лог.Информация("Активация версии OneScript %1", ИспользуемаяВерсия);

	ПроверитьНаличиеИспользуемойВерсии(
		ИспользуемаяВерсия,
		ВерсияКУстановке,
...
Процедура ПроверитьНаличиеИспользуемойВерсии(
	Знач ИспользуемаяВерсия,
	Знач ВерсияКУстановке,
	Знач ВыполнятьУстановкуПриНеобходимости)
	...
	Если ВыполнятьУстановкуПриНеобходимости Тогда
		...
		УстановщикOneScript.УстановитьOneScript(ВерсияКУстановке, , ДополнительныеПараметры);
	Иначе
