...
// а также сборки с сервера сборок в формате branch:<ветка> или pr:<номер>,
// с необязательным номером сборки через решетку: branch:develop#777
//
// Параметры:
...
Процедура ПроверитьКорректностьПереданнойВерсии(Знач ВерсияКУстановке)
	Если НЕ ЭтоДопустимыйНомерВерсии(ВерсияКУстановке)
		...
		И Не ВерсииOneScript.ЭтоКорректнаяСпецификацияСборки(ВерсияКУстановке) Тогда
