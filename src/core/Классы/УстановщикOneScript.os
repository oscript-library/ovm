...
// Допустимо использовать трех-разрядные номера версий, шорткаты dev и stable,
// а также сборки с сервера сборок в формате build:<ветка или пул-реквест>
//
// Параметры:
...
Процедура УстановитьOneScript(
	...
	Если НЕ ЗначениеЗаполнено(АлиасВерсии) Тогда
		АлиасВерсии = ВерсииOneScript.АлиасПоУмолчанию(ВерсияКУстановке);
	КонецЕсли;
...
Функция СкачатьФайлУстановщика(Знач ВерсияКУстановке, Знач ИспользоватьХ64, Знач ИспользоватьFDD)
	...
	ПараметрыСкачивания = ПолучитьПараметрыСкачиванияФайла(ВерсияКУстановке, ИспользоватьХ64, ИспользоватьFDD);

	Соединение = ФабрикаИнтернетСоединения.Создать(ПараметрыСкачивания.Сервер);

	Ресурс = ПараметрыСкачивания.Ресурс;
...
Процедура ПроверитьКорректностьПереданнойВерсии(Знач ВерсияКУстановке)
	Если НЕ ЭтоДопустимыйНомерВерсии(ВерсияКУстановке)
		И Не ВерсииOneScript.ДопустимыеАлиасы().СодержитКлюч(ВерсияКУстановке)
		И Не ЭтоКорректнаяСпецификацияСборки(ВерсияКУстановке) Тогда
...
Функция ЭтоКорректнаяСпецификацияСборки(Знач ВерсияКУстановке)

	Возврат ВерсииOneScript.ЭтоСборкаСервераСборок(ВерсияКУстановке)
		И ЗначениеЗаполнено(ВерсииOneScript.ПараметрыСборки(ВерсияКУстановке).ИмяСборки);

КонецФункции

// Определяет сервер и ресурс, с которых необходимо скачать дистрибутив версии
//
// Параметры:
//   ВерсияКУстановке - Строка - Устанавливаемая версия или спецификация сборки
//   ИспользоватьХ64  - Булево - Устанавливать версию x64
//   ИспользоватьFDD  - Булево - Устанавливать fdd дистрибутив
//
//  Возвращаемое значение:
//   Структура - Параметры скачивания дистрибутива:
//		* Сервер - Строка - Адрес сервера, с которого скачивается дистрибутив
//		* Ресурс - Строка - Адрес файла дистрибутива относительно адреса сервера
//
Функция ПолучитьПараметрыСкачиванияФайла(Знач ВерсияКУстановке, Знач ИспользоватьХ64, Знач ИспользоватьFDD)

	ОпределительДистрибутива = КоробочкаСОпределителями.Достать()
		.ИспользоватьХ64(ИспользоватьХ64)
		.ИспользоватьFDD(ИспользоватьFDD);

	Если ВерсииOneScript.ЭтоСборкаСервераСборок(ВерсияКУстановке) Тогда

		// Сервер сборок собирает только дистрибутивы OneScript 2.x
		ОпределительДистрибутива.ЭтоДистрибутивНеткор(Истина);

		ПараметрыСборки = ВерсииOneScript.ПараметрыСборки(ВерсияКУстановке);
		ПереченьФайлов = ВерсииOneScript.ПолучитьДоступныеВидыДистрибутивовСборки(
			ПараметрыСборки.ИмяСборки,
			ПараметрыСборки.НомерСборки
		);

		АдресСервера = ПараметрыOVM.АдресСервераСборок();
	...
	Иначе
		...
		Если ЭтоДопустимыйНомерВерсии(ВерсияКУстановке) Тогда
			...
			ОпределительДистрибутива.ЭтоДистрибутивНеткор(
				СтроковыеФункции.СравнитьВерсии(ВерсияКУстановке, "2.0") >= 0
			);
		...
		АдресСервера = ПараметрыOVM.АдресСайтаОСкрипт();
	...
	Возврат Новый Структура(
		"Сервер, Ресурс",
		АдресСервера,
		ОпределительДистрибутива.Выбрать(ПереченьФайлов)
	);
...
Функция ПроверитьВозможностьУстановкиX64(ВерсияКУстановке)
	...
	Возврат ВерсииOneScript.ЭтоСборкаСервераСборок(ВерсияКУстановке)
		Или ВерсииOneScript.ДопустимыеАлиасы().СодержитКлюч(ВерсияКУстановке)
