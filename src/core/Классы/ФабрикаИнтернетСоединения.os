...
// Создать соединение с указанным сервером
//
// Параметры:
//   АдресСервера - Строка - Адрес сервера. По умолчанию используется адрес сайта OneScript
//
//  Возвращаемое значение:
//   HTTPСоединение - Соединение с сервером
//
Функция Создать(Знач АдресСервера = "") Экспорт
	Если Не ЗначениеЗаполнено(АдресСервера) Тогда
		АдресСервера = АдресСайтаОСкрипт;
	КонецЕсли;
